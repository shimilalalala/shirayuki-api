import { getAnixoEpisodeSources } from '../scraper/episode-sources.js';
...
const buildProxyUrl = (c, m3u8, referer) => {
  ...
  return `${origin}/api/v2/anixo/proxy?url=${encodeURIComponent(m3u8)}&ref=${encodeURIComponent(referer)}`;
};
...
export const anixoEpisodeSourcesController = async (c) => {
  try {
    ...
    const data = await getAnixoEpisodeSources({ animeEpisodeId, ep, server, category });
