...
// AniList has no per-episode metadata; the Anixo player itself pulls episode
// titles from Jikan (MAL). We do the same: AniList id -> idMal -> Jikan episodes.
export const getAnixoEpisodes = async ({ animeId, page } = {}) => {
  ...
  if (!media.idMal) {
    ...
    const episodes = Array.from({ length: count }, (_, i) => ({
      ...
      malId: null,
      filler: false,
      url: `${ANIXO_BASE_URL}/watch/${id}?ep=${i + 1}`,
    }));

    return {
      source: `${ANIXO_BASE_URL}/watch/${id}`,
      animeId: String(id),
      title,
      totalEpisodes: count,
      hasNextPage: false,
  ...
  const episodes = (jikan?.data || []).map((ep) => ({
    ...
    url: `${ANIXO_BASE_URL}/watch/${id}?ep=${ep.mal_id}`,
  }));
