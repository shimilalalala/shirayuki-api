...
export const getAnimexSearch = async ({ q, page } = {}) => {
  ...
  return {
    source: `${ANIMEX_BASE_URL}/search?keyword=${encodeURIComponent(keyword)}`,
    query: keyword,
    pagination,
    results,
  };
...
export const getAnimexSearchAdvanced = async ({ q, type, genres, season, year, status, sort, page } = {}) => {
  const keyword = String(q || '').trim();
  const normalizedPage = normalizePage(page);
...
const SUGGESTION_QUERY = `
  query AnimexSuggestion($search: String) {
...
export const getAnimexSearchSuggestion = async ({ q } = {}) => {
  const keyword = requireQuery(q);
