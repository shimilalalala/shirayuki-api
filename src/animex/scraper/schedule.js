...
const SCHEDULE_QUERY = `
  query AnimexSchedule($start: Int, $end: Int, $page: Int) {
...
export const getAnimexSchedule = async ({ date, timezone, page } = {}) => {
  ...
  const results = schedules
    ...
    .map((s) => {
      ...
      return {
        ...
        href: `/watch/${media.id}`,
        url: `${ANIMEX_BASE_URL}/watch/${media.id}`,
      };
    });

  return {
    source: `${ANIMEX_BASE_URL}/schedule`,
    date: targetDate,
    timezone: zone,
